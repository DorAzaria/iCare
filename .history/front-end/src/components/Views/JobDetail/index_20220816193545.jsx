...
class ViewJobDetail extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      votes_user_len: 0,
      comments: [],
      //comment : comment_number, description, from_number, from_name , replys
    };
  }
  ...
  render() {
    ...
    const makeCommentElement = (comment, id) => {
      console.log(id);
      const { [KEY_NUMBER_COMMENT]: key } = comment;
      let key_comment = "comment_" + key;
      return (
        <div key={key_comment} style={{marginBottom: "40px"}}>
          <PartialComment comment={comment} />
          <div className="leave-reply-div" style={{height: "150px"}}>
            <Button
              color="secondary"
              className="Button_navigation"
              onClick={() => actionSaveReply(id)}
              style={{marginBottom:"15px", marginTop: "15px"}}
            >
              Reply Above Comment
            </Button>
            <textarea
              style={{ width: "100%", border: "1px solid lightskyblue" }}
              onChange={setReplyValue(id)}
            ></textarea>
          </div>
        </div>
      );
    ...
    const renderA = () => {
      ...
      const makeJobElement = (job) => {
        return <PartialJobPost job={job} user={user} page_type={"detail"} style={{width:"1000px"}} />;
      };
      const elementJob = makeJobElement(job);
      const elementsComment = comments.map(makeCommentElement);

      const body = (
        <div className="ViewJobsDetail container" style={{maxWidth: 1200, marginLeft: "150px"}}>
          {elementJob}
          <div className="leave-comment-div mt-5">
            <div style={{ margin: "20px 2px 2px 2px" }}>
              <Button
                className="Button_navigation"
                onClick={actionSaveComment}
              >
                Leave New Comment
              </Button>
              <Button
                className="Button_navigation"
                color={isDisabled ? "secondary" : "secondary"}
                style={{
              ...
              <span style={{ color: "black" }}>
                Voted Number: {votes_all_len}
              </span>
              <div className="mt-3">
                <textarea
                  style={{ width: "100%", border: "1px solid plum" }}
                  onChange={setValue("comment_description")}
                ></textarea>
              </div>
            </div>
          </div>
          <div className="mt-4">
            <div className="ViewJobsBabysitter_titleAll">
              <span className="Title_styleA" style={{marginLeft: "120px"  }}>{titleJobComments}</span>
            </div>
            <div className="ViewJobsBabysitter_listAll">{elementsComment}</div>
          </div>
        </div>
      );
