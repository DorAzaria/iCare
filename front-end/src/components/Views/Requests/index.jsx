  ...
  render() {
        ...
        <div>
          <div className="Layout_inputLabel">
            ...
            <input
              style={{ width: "100%" }}
              type="datetime-local"
              onChange={setTime("timeB")}
            />
          </div>
        </div>
        <button className="Button_navigation" onClick={actionSubmit}>
          {labelSubmit}
        </button>
    ...
    const renderA = () => {
      ...
      const makePostedJobElement = (application) => {
        const { [KEY_NUMBER_APPLICATION]: numberApplication } = posted_jobs;
        ...
        return (
          <div key={posted_key}>
            ...
            <Link to={chatLink} className="Button_navigation">
              CHAT
            </Link>
          </div>
        );
      ...
      const makePostedJobsElement = (jobApplication) => {
        ...
        const { [KEY_NUMBER_JOB]: key, [KEY_APPLICATIONS]: applications } =
          jobApplication;
        ...
        return (
          <div
            key={application_key}
            className="ViewRequestsParent_singleApplication"
          >
      ...
      const makeAppliedJobElement = (jobApplication) => {
        ...
        if (numberChat) {
          ...
          chatLink = (
            <Link to={link} className="Button_navigation">
              CHAT
            </Link>
          );
        } else {
        ...
        return (
          <div
            key={applied_job_key}
            className="ViewRequestsBabysitter_singleApplication"
          >
      ...
      const body = (
        <div className="ViewRequestsParent">
          ...
          <div className="ViewRequestsParent_listAll">{elementsPostedJob}</div>
