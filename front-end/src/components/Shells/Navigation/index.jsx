...
import { Navbar, NavbarBrand, Nav, NavLink, NavbarToggler, Collapse, NavItem, Dropdown, DropdownToggle, DropdownMenu, DropdownItem } from 'reactstrap';
...
class ShellNavigation extends React.Component {
    ...
    DatabaseDriver.loadNotifications(parameters)
      .then((data) => {
        ...
        data.map((item) => {
          ...
          item.image = item.image ? SERVER_PROFILE_URL + item.image : 'logo.png'
        })
  ...
  render () {
    ...
    const navigationLinks = links.map((pair) => {
      ...
      return (
        <NavItem>
          <NavLink key={ key } href={ link } style={{backgroundColor: '#1d4354', color: 'white', paddingLeft: '10px'}}>{ label }</NavLink>
        </NavItem>
      );
    ...
    return (
      ...
      <header className='sticky-top icare-navbar'>
        <Navbar dark style = {{height:70}} expand="md" className="container-xxl">
          <NavbarBrand href = "/">
              ...
              <Nav className = "mr-auto" navbar>
                ...
                <Dropdown isOpen={this.state.dropdownOpen} toggle={this.toggleDrop}>
                  <DropdownToggle aria-expanded data-toggle="dropdown" tag="span">
                    <img src = {user.avatar ? profileUrl : 'logo.png'} className = "profile-avatar" alt = "user profile"></img>
                  </DropdownToggle>
                  <DropdownMenu end>
                    <DropdownItem>
                    ...
                    <DropdownItem>
                      <Link to='/log-out' style={{textDecoration: 'none', color: 'black'}}>Log out</Link>
                    </DropdownItem>
                  </DropdownMenu>
                </Dropdown>
              </Nav>
